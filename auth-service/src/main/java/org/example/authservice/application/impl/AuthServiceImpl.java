...
import org.example.authservice.config.PasswordEncoderConfig;
...
public class AuthServiceImpl implements AuthService {
    ...
    public void registerMember(MemberRegistRequestDto registryRequestDto) {
        ...
        MemberCreateRequest createRequest = new MemberCreateRequest(
                ...
                passwordEncoder.passwordEncoder().encode(registryRequestDto.getPassword()), // 비밀번호 해싱
    ...
    public TokenResponse login(LoginRequestDto requestDto) {
        ...
        if (member == null || member.password() == null) {
            throw new IllegalArgumentException("존재하지 않는 회원이거나 정보가 부족합니다.");
        }

        // 비밀번호 검증
        if (!passwordEncoder.passwordEncoder().matches(requestDto.password(), member.password())) {
            throw new IllegalArgumentException("이메일 또는 비밀번호가 올바르지 않습니다.");
        }

        List<String> roles = member.roles() != null && !member.roles().isEmpty()
                ? member.roles()
                : List.of("ROLE_USER");
